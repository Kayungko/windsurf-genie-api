...
2. 系统已安装或可以安装：
   ...
   - `npm`
3. 已准备好代码仓库地址，例如：
   - SSH：`git@github.com:Kayungko/windsurf-genie-api.git`
   - HTTPS：`https://github.com/Kayungko/windsurf-genie-api.git`
...
```bash
cd /opt
git clone git@github.com:Kayungko/windsurf-genie-api.git windsuf-genie-api
...
> 建议服务仅监听内网地址（如 127.0.0.1:8082），通过 1Panel/openresty 做反向代理对外暴露域名和 HTTPS。
...
2. 在网站 / 反向代理管理中：
   ...
   - 反向代理目标地址设置为：`http://127.0.0.1:8082`。
3. 如需 HTTPS：
   - 在 1Panel 中为该域名申请或上传证书。
   - 启用 HTTPS，并将流量代理到 `127.0.0.1:8082`。

完成后，外部访问流程为：

```text
客户端 → https://api.example.com → openresty(1Panel) → http://127.0.0.1:8082 (windsurf-genie-api)
```
...
```text
Windsurf Genie API listening on port 8082
```

#### 7.2 本机验证（服务器内）

```bash
curl http://127.0.0.1:8082/random-user
curl http://127.0.0.1:8082/bin/411111
```
